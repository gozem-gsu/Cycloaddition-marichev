    ...
    with open(ouptut_file, "w") as opt_xyz_file:
        for coord in formatted_coordinates:
            ...
            if len(coord) == 4:
                ...
                print(f"Coordinates saved to: {ouptut_file}")
            else:
                print(f"No log file found in {input_file}")
...
#             else f"{folder}.log"
#         )
#         input_file = os.path.join(folder, subdir, log_file_name)
#         ouptut_file = os.path.join(
#             "xyz-opt",
#             f"{folder}-opt-an.xyz"
#             if subdir == "anion"
